...
end_date = datetime(2020, 12, 31)   # Date de fin
output_dir = '/home/gaia/Documents/processing_1_sec/double_duration_speed'  # Répertoire de sortie
...
def find_peak_edges(df_ratio, peak, threshold=0.001):
    ...
    while start_index > 0 and (df_ratio['ratio_1_8Hz'].iloc[start_index] >= df_ratio['ratio_1_8Hz'].iloc[start_index - 1] or
                               df_ratio['ratio_1_8Hz'].iloc[peak] - df_ratio['ratio_1_8Hz'].iloc[start_index] <= threshold):
        start_index -= 1
        
    # Trouver la fin du pic
    while end_index < len(df_ratio['ratio_1_8Hz']) - 1 and (df_ratio['ratio_1_8Hz'].iloc[end_index] >= df_ratio['ratio_1_8Hz'].iloc[end_index + 1] or
                               df_ratio['ratio_1_8Hz'].iloc[peak] - df_ratio['ratio_1_8Hz'].iloc[end_index] <= threshold):
        end_index += 1
        
    return start_index, end_index

def process_data(file_A, file_E, output_file):
    ...
    df_E['time_UTC'] = pd.to_datetime(df_E['time_UTC'])

    # Calculer le ratio
    df_ratio = pd.DataFrame()
    df_ratio['time_UTC'] = df_E['time_UTC']
    df_ratio['ratio_1_8Hz'] = df_E['RSAM_env_smooth_1-8Hz'] / df_A['RSAM_env_smooth_1-8Hz']

    # Détection des pics avec les nouveaux paramètres sur le ratio brut
    peaks_1_8Hz, properties_1_8Hz = find_peaks(df_ratio['ratio_1_8Hz'], prominence=prominence, distance=distance_min)
    ratio = df_ratio['ratio_1_8Hz'].iloc[peaks_1_8Hz].values
    ...
    # Boucle pour chaque pic détecté
    for i_w, peak_w in enumerate(peaks_1_8Hz):
        # Utiliser l'indice i pour accéder à peak_amplitudes
        threshold_w = 0.5 * ratio[i_w]

        # Trouver le début du pic
        start_index_w = peak_w
        while start_index_w > 0 and df_ratio['ratio_1_8Hz'].iloc[start_index_w] >= threshold_w:
            start_index_w -= 1
        ...
        # Trouver la fin du pic
        end_index_w = peak_w
        while end_index_w < len(df_ratio['ratio_1_8Hz']) - 1 and df_ratio['ratio_1_8Hz'].iloc[end_index_w] >= threshold_w:
            end_index_w += 1

        # Assurer que end_index ne sort pas des limites
        if end_index_w >= len(df_ratio['ratio_1_8Hz']):
            end_index_w = len(df_ratio['ratio_1_8Hz']) - 1
    ...
    peaks_df = pd.DataFrame({
        'Peak_Time_UTC': df_ratio['time_UTC'].iloc[peaks_1_8Hz],
        'Ratio': ratio,
        'RSAM_A': df_A['RSAM_env_smooth_1-8Hz'].iloc[peaks_1_8Hz].values,
        'RSAM_E': df_E['RSAM_env_smooth_1-8Hz'].iloc[peaks_1_8Hz].values,
        'Duration': durations,
...
for single_date in pd.date_range(start_date, end_date):
    ...
    chemin_vers_les_fichiers_A = f'/home/gaia/Documents/processing_1_sec/rsam/rsam_STRA_{date_str}*.csv'
    chemin_vers_les_fichiers_E = f'/home/gaia/Documents/processing_1_sec/rsam/rsam_STRE_{date_str}*.csv'
    ...
    if fichiers_A and fichiers_E:
        output_file = os.path.join(output_dir, f'peaks_data_{date_str}.csv')
        process_data(fichiers_A[0], fichiers_E[0], output_file)
