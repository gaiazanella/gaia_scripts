...
t_init=UTCDateTime("2019-01-01T00:00:00")
t_end=UTCDateTime("2020-01-01T00:00:00")
#save_dir = '/home/gaia/Documents/processing_10_sec/2019/rsam_test'
save_dir = '/home/dario/Documenti/processing_10_sec/2019/rsam_test/'
db = '/mnt/bigmama3'
#stz = ['STR1']
...
for j in range (len(stz)) :
    for i in range (len(th_day)-1): 
        stb3 = client.get_waveforms(network=net[0], station=stz[j], location = "", channel=channel[0], starttime=UTCDateTime(th_day[i]), endtime=UTCDateTime(th_day[i+1]))
        print(stb3)
        stb4 = client.get_waveforms(network=net[0], station=stz[j], location = "", channel=channel[0], starttime=UTCDateTime(th_day[i]), endtime=UTCDateTime(th_day[i+1]))
        print(stb4)
...
# Parcourir chaque multiple de 10 secondes de la période
    ...
    # Fenêtre de 5 secondes avant et 5 secondes après (10 secondes en tout)
            window_start = current_time - 5  # 5 secondes avant la seconde exacte
            window_end = current_time + 5  # 5 secondes après la seconde exacte
...
############################### B3
    # Si l'index est valide, extraire les données pour cette fenêtre
        ...
        # Si la fenêtre est complète (exactement 10 secondes de données)
                if len(window_datab3) == 10 * sampling_rate:
...
############################### B4
    # Si l'index est valide, extraire les données pour cette fenêtre
        ...
        # Si la fenêtre est complète (exactement 10 secondes de données)
                if len(window_datab4) == 10 * sampling_rate:
    ...
    # Passer au prochain multiple de 10 secondes
            current_time += 10  # Incrémenter de 10 secondes
