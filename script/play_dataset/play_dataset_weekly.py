...
# 2ème subplot : durée moyenne hebdomadaire des glissements de terrain (auto)
axs[1].plot(daily_duration_mean_rolling.index, daily_duration_mean_rolling.values, label='Weekly Average Duration (Auto)', color='green', linewidth=2)
axs[1].set_ylabel('Average Duration (s)')
axs[1].legend()
axs[1].grid(True)

# 3ème subplot : durée maximale hebdomadaire des glissements de terrain (auto)
axs[2].plot(daily_duration_max_rolling.index, daily_duration_max_rolling.values, label='Weekly Max Duration (Auto)', color='red', linewidth=2)
axs[2].set_ylabel('Max Duration (s)')
axs[2].legend()
axs[2].grid(True)

# 4ème subplot : valeur moyenne hebdomadaire de 'RSAM_E' (auto)
axs[3].plot(daily_rsam_mean_rolling.index, daily_rsam_mean_rolling.values, label='Weekly Average RSAM_E (Auto)', color='blue', linewidth=2)
axs[3].set_ylabel('Average RSAM_E')
axs[3].legend()
axs[3].grid(True)

# 5ème subplot : valeur maximale hebdomadaire de 'RSAM_E' (auto)
axs[4].plot(daily_rsam_max_rolling.index, daily_rsam_max_rolling.values, label='Weekly Max RSAM_E (Auto)', color='purple', linewidth=2)
