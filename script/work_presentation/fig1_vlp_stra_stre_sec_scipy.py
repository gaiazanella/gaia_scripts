...
from scipy.signal import spectrogram  # Importer la fonction pour le spectrogramme
...
ti = UTCDateTime("2020-03-19T04:55:00.000")
tf = ti + (60 * 10 * 1 * 1)  # 5 minutes de données 
...
starttimea = sta[0].stats.starttime
starttimee = ste[0].stats.starttime

# Vecteur temps en secondes depuis le début des données
...
f, x, Sxx = spectrogram(signal, fs)

# Tracer le spectrogramme dans le 4ème subplot
axs[3].pcolormesh(x, f, 10 * np.log10(Sxx), shading='auto')
axs[3].set_ylabel('Fréquence [Hz]')
axs[3].set_xlabel('Temps [sec]')
# axs[3].set_title('Spectrogramme de la trace sismique STRA')
# axs[3].colorbar(label='Puissance [dB]')
axs[3].grid(True)
