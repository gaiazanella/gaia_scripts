...
# 2ème subplot : durée moyenne quotidienne des glissements de terrain (auto) et sa moyenne mobile hebdomadaire
...
# 3ème subplot : durée maximale quotidienne des glissements de terrain (auto) et sa moyenne mobile hebdomadaire
...
# 4ème subplot : valeur moyenne quotidienne de 'RSAM_E' (auto) et sa moyenne mobile hebdomadaire
axs[3].plot(daily_rsam_mean.index, daily_rsam_mean.values, label='Daily Average RSAM_E (Auto)', color='skyblue', linewidth=2)  # Couleur plus foncée pour daily
axs[3].plot(daily_rsam_mean_rolling.index, daily_rsam_mean_rolling.values, label='Weekly Average RSAM_E (Auto)', color='darkblue', linewidth=2)  # Couleur plus claire pour weekly
axs[3].set_ylabel('Average RSAM_E')
axs[3].legend()
axs[3].grid(True)

# 5ème subplot : valeur maximale quotidienne de 'RSAM_E' (auto) et sa moyenne mobile hebdomadaire
axs[4].plot(daily_rsam_max.index, daily_rsam_max.values, label='Daily Max RSAM_E (Auto)', color='violet', linewidth=2)  # Couleur plus foncée pour daily
axs[4].plot(daily_rsam_max_rolling.index, daily_rsam_max_rolling.values, label='Weekly Max RSAM_E (Auto)', color='darkviolet', linewidth=2)  # Couleur plus claire pour weekly
